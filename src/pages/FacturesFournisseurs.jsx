...
// tous projets confondus (contrairement à l'onglet "Factures frs" d'un
...
// mise en évidence (tab + focusId, voir ProjetDetail.jsx). Pour les achats
// non liés à un projet (loyer, comptabilité...), voir la page Dépenses.
const STATUTS = ['À payer', 'Payée']
const STATUT_STYLE = {
  'À payer': { bg: '#FFF7ED', color: '#EA580C' },
...
export default function FacturesFournisseurs() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true)
      const { data } = await supabase.from('factures_frs')
        .select('id, numero, montant_ht, statut, date_facture, date_echeance, projet_id, projets(nom), fournisseurs(nom)')
        .is('deleted_at', null)
  ...
  function aller(f) {
    navigate('/projets/' + f.projet_id, { state: { tab: 'factures_frs', focusId: f.id } })
  }

  const filtrees = factures.filter(f => {
    const matchSearch = !search ||
      f.numero?.toLowerCase().includes(search.toLowerCase()) ||
      f.fournisseurs?.nom?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div style={{ padding: isMobile ? 14 : 24, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>Factures fournisseurs</h2>
        <div style={{ fontSize: 12, color: '#9CA3AF', marginTop: 2 }}>Toutes les factures fournisseurs liées à un projet, tous projets confondus</div>
      </div>

      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'À payer', value: fmt(aPayer), color: '#EA580C', bg: '#FFF7ED' },
      ...
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: 14, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 10 }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="N°, fournisseur, projet..."
      ...
      {loading ? <div style={{ textAlign: 'center', padding: 40, color: '#9CA3AF' }}>Chargement...</div>
        : filtrees.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#9CA3AF', background: '#F9FAFB', borderRadius: 12, border: '2px dashed #E5E7EB' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 500 }}>Aucune facture fournisseur</div>
          </div>
        ) : isMobile ? (
                ...
                {filtrees.map((f, i) => {
                  ...
                  return (
                    <tr key={f.id} onClick={() => aller(f)}
                      ...
                      <td style={{ padding: '11px 14px', fontWeight: 500, color: '#111827' }}>{f.fournisseurs?.nom || '—'}</td>
                      ...
                      <td style={{ padding: '11px 14px', textAlign: 'right', fontWeight: 600, color: '#EA580C' }}>{fmt(f.montant_ht)}</td>
