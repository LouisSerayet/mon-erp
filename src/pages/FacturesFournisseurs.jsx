...
export default function FacturesFournisseurs() {
  ...
  return (
    <div style={{ padding: isMobile ? '28px 18px 60px' : '48px 40px 80px', fontFamily: fonts.display, color: colors.ink, maxWidth: 1180, margin: '0 auto' }}>
      ...
      <h1 style={{ margin: '14px 0 0', fontSize: isMobile ? 26 : 34, fontWeight: 700, letterSpacing: '-0.015em' }}>Factures fournisseurs</h1>
      <p style={{ color: colors.inkMuted, fontSize: 13, margin: '10px 0 0' }}>Toutes les factures fournisseurs liées à un projet, tous projets confondus</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr 1fr' : 'repeat(3, 1fr)', gap: 24, margin: '36px 0 32px', paddingBottom: 24, borderBottom: '1px solid ' + colors.line }}>
        <div>
          ...
          <div style={{ fontFamily: fonts.mono, fontSize: 24, fontWeight: 500, marginTop: 8, fontVariantNumeric: 'tabular-nums' }}>{fmt(aPayer)}</div>
        </div>
        <div>
              ...
              {filtrees.map(f => {
                ...
                return (
                  <tr key={f.id} onClick={() => aller(f)}
                    ...
                    <td style={{ padding: '12px 14px 12px ' + (enRetardF ? '12px' : '0'), fontWeight: 500 }}>{f.fournisseurs?.nom || '—'}</td>
