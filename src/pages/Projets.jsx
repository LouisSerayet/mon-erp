...
export default function Projets() {
  ...
  const [form, setForm] = useState({ code: '', nom: '', statut: 'En cours', montant_ht: '', markup: '', type_projet: '', service: '' })
  ...
  async function fetchProjets() {
    setLoading(true)
    const { data } = await supabase.from('projets').select('*, clients(raison_sociale)').order('created_at', { ascending: false })
    setProjets(data || [])
    setLoading(false)
  }
  ...
  async function saveProjet(e) {
    ...
    setSaving(false)
    setShowForm(false)
    setForm({ code: '', nom: '', statut: 'En cours', montant_ht: '', markup: '', type_projet: '', service: '' })
    fetchProjets()
  }
    ...
    <div style={{ padding: 24 }}>
              ...
              {filtered.map(p => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '11px 14px', fontWeight: 500 }}>{p.nom}</td>
                    <td style={{ padding: '11px 14px', color: '#555' }}>{p.clients ? p.clients.raison_sociale : ''}</td>
                    <td style={{ padding: '11px 14px' }}>
                      <span style={{ background: ps.bg, color: ps.color, padding: '3px 10px', borderRadius: 10, fontSize: 12, fontWeight: 500 }}>
                        {p.statut}
                      </span>
                    </td>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: 28, width: 480, maxHeight: '90vh', overflow: 'auto' }}>
            <h2 style={{ fontSize: 17, fontWeight: 500, marginBottom: 20 }}>Nouveau projet</h2>
            <form onSubmit={saveProjet}>
              {[
                { label: 'Code projet', key: 'code', placeholder: 'ex: LST090071717', required: true },
                { label: 'Nom du projet', key: 'nom', placeholder: 'ex: Amenagement bureaux', required: true },
                { label: 'Montant HT (EUR)', key: 'montant_ht', placeholder: '0', type: 'number' },
                { label: 'Markup (%)', key: 'markup', placeholder: '21', type: 'number' },
                { label: 'Type de projet', key: 'type_projet', placeholder: 'ex: Design and Build' },
                { label: 'Service', key: 'service', placeholder: 'ex: FR-Design&Build' },
              ].map(f => (
